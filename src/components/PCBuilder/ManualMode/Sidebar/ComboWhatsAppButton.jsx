...
const ComboWhatsAppButton = ({ selectedCount, pcBuild, totalPrice }) => {
  ...
  return (
    <button
      ...
      className={`w-full py-[11px] px-[14px] rounded-xl border-none font-sans font-extrabold text-xs tracking-tight flex items-center justify-center gap-[7px] transition-all duration-200 relative overflow-hidden ${
        selectedCount < 2 
          ? 'bg-slate-800 text-slate-600 border border-slate-700 cursor-not-allowed' 
          : 'text-white cursor-pointer whatsapp-btn-gradient'
      }`}
    >
      {selectedCount >= 2 && (
        <div 
          className="absolute top-0 left-0 bottom-0 transition-all duration-500 whatsapp-progress-gradient"
          style={{ width: `${(selectedCount / 8) * 100}%` }}
        />
      )}
      <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24" className="flex-shrink-0 relative">
