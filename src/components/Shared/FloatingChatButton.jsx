  ...
  const ConditionsModal = () => (
    ...
    >
      ...
      >
        ...
        <button
          onClick={() => setShowConditionsModal(false)}
          className="absolute top-4 right-4 z-10 bg-white/10 hover:bg-white/20 rounded-full p-2 transition-all duration-200 hover:scale-110 border border-white/20"
        >
  ...
  return (
    <>
      ...
      >
        ...
        {/* --- MENÚ EXPANDIDO --- */}
        {isExpanded && (
          <div className="relative flex flex-col items-stretch gap-3.5 mb-3 w-[350px] sm:w-auto">
            
            {/* WhatsApp */}
            <button
              onClick={handleWhatsApp}
              className="group flex items-center gap-3.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-4 py-3.5 rounded-2xl shadow-xl hover:shadow-green-500/50 border border-white/20"
            >
              <div className="w-14 h-14 bg-white/25 rounded-xl flex items-center justify-center border border-white/30 group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
            ...
            <button
              ...
              className="sm:hidden group flex items-center gap-3.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white px-4 py-3.5 rounded-2xl shadow-xl hover:shadow-cyan-500/50 border border-white/20"
            >
              <div className="w-14 h-14 bg-white/25 rounded-xl flex items-center justify-center border border-white/30 flex-shrink-0">
            ...
            <button
              ...
              className="sm:hidden group flex items-center gap-3.5 bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white px-4 py-3.5 rounded-2xl shadow-xl hover:shadow-orange-500/50 border border-white/20"
            >
              <div className="w-14 h-14 bg-white/25 rounded-xl flex items-center justify-center border border-white/30 flex-shrink-0">
              ...
              <div className="flex-1 min-w-0 text-left">
                ...
                <div className="text-xs leading-tight text-white/75 mt-1">Información Importante</div>
              </div>
              <Send className="w-5 h-5 text-white/60 group-hover:text-white/90 group-hover:translate-x-1 transition-all duration-300 flex-shrink-0" strokeWidth={2.5} />
            </button>
          </div>
        ...
        <button
          ...
          className="group relative bg-gradient-to-r from-green-500 via-green-600 to-green-700 hover:from-green-600 hover:via-green-700 hover:to-green-800 text-white rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:shadow-green-500/50 overflow-hidden"
        >
          {/* Efecto Shine - Solo desktop */}
          ...
          {/* Contenido */}
          <div className={`relative flex items-center gap-2 sm:gap-4 p-3 sm:p-4 ${isExpanded ? 'expanded' : ''}`}>
            
            {/* Icon Container */}
            <div className="relative transition-all duration-300">
              <div className="w-8 h-8 sm:w-12 sm:h-12 bg-white/20 rounded-md sm:rounded-lg flex items-center justify-center border border-white/30">
                {isExpanded ? (
            ...
            {/* Arrow Icon */}
            <div className="hidden sm:block transition-transform duration-300">
              <Send className="w-4 h-4 text-white/80" strokeWidth={2.5} />
            </div>
          </div>

          {/* Accent Line - Solo desktop */}
          <div className="hidden sm:block absolute bottom-0 left-0 right-0 h-0.5 sm:h-1 bg-gradient-to-r from-transparent via-white to-transparent animate-accent-line" />
        </button>
      </div>
    </>
  );
